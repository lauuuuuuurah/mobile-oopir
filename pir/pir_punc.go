...
import (
	...
	"github.com/lauuuuuuurah/mobile-oopir/psetggm"
)
...
func (resp *PuncHintResp) InitClient_perfTest(source *rand.Rand) Client {
	c := PuncClient{
		randSource: source,
		nRows:      resp.NRows,
		RowLen:     resp.RowLen,
...
func (c *PuncClient) initSets_perfTest() {
	c.sets = make([]SetKey, len(c.hints))
	c.IdxToSetIdx = make([]int32, c.nRows)

	nMapPart := c.nRows / 4
	IdxToSetIdx2 := make([][]int32, 4)
	for i := range IdxToSetIdx2 {
		idxMap := make([]int32, nMapPart)
		for j := 0; j < nMapPart; j++ {
			idxMap[j] = -1
		}
		IdxToSetIdx2[i] = idxMap
	}
	var pset PuncturableSet
	...
	for i := 0; i < len(c.hints); i++ {
		...
		for _, j := range pset.elems {
			// if _, contains := c.IdxToSetIdx2[int32(j)]; !contains {
			// 	c.IdxToSetIdx2[int32(j)] = int32(i)
			// }
			if IdxToSetIdx2[j/nMapPart][j%nMapPart] == -1 {
				IdxToSetIdx2[j/nMapPart][j%nMapPart] = int32(i)
				//counter[0] += int32(j)
...
func (resp *PuncHintResp) InitClient2(source *rand.Rand, percent float64) Client {
	c := PuncClient{
		randSource: source,
		nRows:      resp.NRows,
		RowLen:     resp.RowLen,
...
func (c *PuncClient) initSets2(threshold float64) {
	c.sets = make([]SetKey, len(c.hints))
	c.IdxToSetIdx = make([]int32, c.nRows)
	for i := range c.IdxToSetIdx {
		c.IdxToSetIdx[i] = -1
